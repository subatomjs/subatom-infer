...
> {
  ...
  _parse(input: unknown, ctx: ParseContext): DynamicParseReturnType<any> {
    ...
    if (asyncChecks.length > 0) {
      ...
      return Promise.all(asyncChecks).then((results) => {
        for (const res of results) {
          if (res.success) return makeSuccess(res.data);
        }
        addIssue(ctx, {
          code: "invalid_union",
  ...
  constructor(
    ...
    for (const opt of options) {
      ...
      if (!field || !("value" in field)) {
        throw new Error(
          `Every discriminated union member must specify literal on "${discriminator}"`,
        );
      }
      this.optionMap.set((field as { value: unknown }).value, opt);
    }
  }
  ...
  _parse(
    ...
    if (!targetSchema) {
      addIssue(ctx, {
        code: "invalid_value",
        received: discValue,
        message: `No matching branch for discriminator ${
          this.discriminator
        }="${String(discValue)}"`,
      });
      return makeFailure(ctx.issues);
    ...
    return targetSchema._parse(input, ctx);
  }
}
...
> {
  ...
  _parse(
    ...
    // Async handling with isolated branch contexts to prevent issue collisions
